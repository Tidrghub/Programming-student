...
def test_pretty_print():
    ...
    test_inputfile = "pe_6_2_generated.txt"
    print(f"Voor de test wordt bestand {test_inputfile} aangemaakt, met de volgende inhoud: ")
    try:
        with open(test_inputfile, 'w') as dummy_file:
            for number, name in cards.items():
                dummy_file.write(f"{number}, {name}\n")
                print(f"{number}, {name}\n", end="")
    except:
        print(f"Fout: bestand {print(traceback.format_exc())} kon niet worden aangemaakt. Python-error:")
    ...
    builtins.open = lambda file, mode='r', buffering=-1, encoding=None, errors=None, newline=None, closefd=True, opener=None: \
        original_open("pe_6_2_generated.txt", mode=mode, buffering=buffering, encoding=encoding, errors=errors, newline=newline, closefd=closefd, opener=opener)
